...
const emptyForm: CreateProductInput = { name: "", price: 0, description: "", category: "General" };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
          <Field label="Nombre" error={errors.name}>
            <input name="name" value={form.name} onChange={handleChange}
              className={inputClass(!!errors.name)} placeholder="Ej: iPhone 13" />
          </Field>
          <Field label="Precio (€)" error={errors.price}>
            <input name="price" type="number" min={0} value={form.price || ""} onChange={handleChange}
              className={inputClass(!!errors.price)} placeholder="Ej: 250" />
          </Field>
          <Field label="Descripción" error={errors.description}>
            <textarea name="description" value={form.description} onChange={handleChange}
              rows={3} className={inputClass(!!errors.description)} placeholder="Describe el producto..." />
          </Field>
          <Field label="Categoría">
            <select name="category" value={form.category} onChange={handleChange} className={inputClass(false)}>
...
function inputClass(hasError: boolean) {
  return `w-full border rounded-xl px-4 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-teal-500 transition-colors ${hasError ? "border-red-400 bg-red-50" : "border-gray-200 bg-gray-50"}`;
}
